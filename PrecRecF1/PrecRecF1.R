...
for (numrows in graph_numrows) { #400
  for (t in c("yes", "no")) {
    ...
    pl
    ggsave(pl, file = sprintf("PrecRecF1/PrecRecF1_n%d_t%s_large%d_xyz%s_%s.pdf", numrows, t, large_int, use_xyz, append_str), width = 4, height = 4)
  }
}
...
for (numrows in graph_numrows) { #400
  ...
  pdf(sprintf("PrecRecF1/test_analysis_n%d_large%d_xyz%s_%s.pdf", numrows, large_int, use_xyz, append_str), width = 3, height = 4)
