...
for (numrows in c(1000*mult)) {#1000
  ...
  for (t in c("yes", "no")) {
    ...
    pl
    ggsave(pl, file = sprintf("FXstrength/FXstrength_PRF_n%d_t%s_mult%d_xyz%s_%s.pdf", numrows, t, mult, use_xyz, append_str), width = 4, height = 4)
   ...
   pl_wrongdir <- readRDS(rds_file) %>%
      ...
      theme(legend.position = "bottom",
            axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
    pl_wrongdir
    ggsave(pl_wrongdir, file = sprintf("FXstrength/FXstrength_direction_n%d_t%s_mult%d_xyz%s_%s.pdf", numrows, t, mult, use_xyz, append_str), height = 3.5, width = 4)
  }
}
